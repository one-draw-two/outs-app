<template>
  <div
    class="-mx-4 px-4 h-12 rounded-xl flex-center bg-repeat-x bg-[length:256px_auto]"
    :style="{ backgroundImage: season?.blueprint?.bgUrl ? `url(${getSanityUrl(season.blueprint?.bgUrl)})` : 'none' }"
  >
    <select v-model="selectedSeason" @change="change" class="stroke-text !not-italic">
