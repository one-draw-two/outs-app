export default function () {
  ...
  if ($capacitor.$platform !== 'web') {
    $capacitor.$pushNotifications.requestPermissions().then(async (result: any) => {
      ...
      if (fcmToken) {
        ...
        await useSecureFetch('push-token-notification', 'token', 'post', { token: fcmToken.token, platform: $capacitor.$platform }) // I need to make sure this gets called event when app is closed on my device
      }
    ...
    $capacitor.$liveActivities.addListener('StartTokenReceived', async (data: any) => {
      ...
      await useSecureFetch('push-token-liveactivity-start', 'token', 'post', getLiveActivityPayload(data.token, data.activityType, data.instanceId)) // I need to make sure this gets called event when app is closed on my device
    })
    ...
    $capacitor.$liveActivities.addListener('UpdateTokenReceived', async (data: any) => {
      ...
      await useSecureFetch('push-token-liveactivity-updateend', 'token', 'post', getLiveActivityPayload(data.token, data.activityType, data.instanceId)) // I need to make sure this gets called event when app is closed on my device
    })
  ...
  const processStartTokenQueue = async () => {
    ...
    await useSecureFetch('push-token-liveactivity-start', 'token', 'post', payload)
