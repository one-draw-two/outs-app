      ...
      <div class="lg:h-16 main-container flex max-lg:flex-col gap-4 lg:items-center">
        <div class="flex items-center justify-between gap-4 px-4 rounded-full py-2" :class="`bg-${stage?.color}-300`">
          ...
          <div class="flex items-center gap-2">
            <NuxtLink v-for="r of rounds" :key="r?.id" :to="getRoundLink(r?.id)" class="relative hover:underline w-5 flex-center">
...
const { data: rounds } = usePSWatch<_P_Round>('SELECT * FROM "calendar_rounds" WHERE _stage = ? ORDER BY sePI ASC', [stage.value?.id], { detectChanges: true })
